...
double powerMethod(int n, double *A, double *result, double *x0, double tolerance, int maxIterations)
{
  double lambda0 = 0.0;
  double lambda1 = 0.0;
  double v0[n];
...
int main()
{
  ...
  for (int i = 0; i < partitions; i++)
  {
    ...
    for (int i = 0; i < n; i++)
      A[i * n + i] -= mu;
    double lambda = inversePowerMethod(n, A, v, x0, tolerance, maxIterations);
    printf("lambda = %f\n", lambda + mu);
    for (int i = 0; i < n; i++)
      A[i * n + i] += mu;
  }
